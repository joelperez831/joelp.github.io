...
#### &nbsp;&nbsp;&nbsp;&nbsp; Rick and Morty can come off as a very dark and nihilistic show. However, according to the sentiment analysis of the show, in its entirety, the show seems to be more netural, with an almost equal amount of negative and positive sentiments.  
...
#### Term frequencies are a large part in text mining. So, let's plot the top 20 words per season and exclude main character names, very common everyday words, aka stop words, and some random words from the subtitle file.    
...
```  
  ...
  We can see from that the top positions for each season are: meeseeks, unity and pickle, which, if you watch Rick and Morty, is not so suprising. One thing I loved is the word "noob" made the list. Noob Noob is a favorite character of mine. Although, he had few screentime, he still managed to make an impact on the episode/season.  
...
ggraph(bigram_graph, layout = "fr") + geom_edge_link(aes(edge_alpha = n),
  ...
  geom_node_point(color = "lightblue", size = 10) + geom_node_text(aes(label =name), vjust = 1, hjust = 1) + theme_void()
```  
## **Summary**